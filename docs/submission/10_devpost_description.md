...
## The executor is swappable — including the hottest OSS agent of 2026

Foreman's executor boundary is one protocol: `execute(task, deps) -> Handoff`. Everything downstream — the pytest gates, the ledger, the dispute layer — grades the *workspace*, not the author. So who writes the code is pluggable, and three backends ship: the native tool-loop (pure Python, default), Alibaba's own **qwen-code** CLI, and NousResearch's **hermes-agent** (MIT, 188k★) in headless mode — pointed at the same DashScope Qwen models. The integration runs **both ways**: Foreman is also packaged as a Hermes *skill* (same agentskills category as its built-in claude-code/codex skills), so a Hermes user who says "here are 15 requirements, build them all" gets a Hermes that knows to delegate the checklist to Foreman and report done/blocked honestly from the ledger. A frontier agent has no verification gate and no crash-proof ledger; Foreman's native executor is deliberately minimal — the pairing gives each side what the other is missing, and the verifier can't tell who wrote the code. That's the point: a rejection is a rejection, whoever the author is.

The native executor also carries six tools now — including `search_files` (grep-style orientation in a real codebase before editing) and `web_search` (live internet via DuckDuckGo, zero keys, failures returned as readable tool results instead of crashes).

## Try it in 60 seconds

No API key needed: check "Demo mode" in the web console (or pass `--mock`) and the entire product runs offline against a scripted fake planner/executor/verifier — parallel runs, dispute/arbitration, stop/resume, per-run cost accounting, all of it, finishing in under a second (slow it for filming with `FOREMAN_MOCK_DELAY`). For a real run with no coupon and no payment: DashScope's **free per-model quota** is enough. Confirmed working on free tier as of submission: `qwen3-max` (planner/verifier/arbiter) and `qwen3-coder-flash` (executor), with `qwen-turbo` as fallback — these are pinned as the repo's defaults, so a judge's clone runs on free quota out of the box. The whole demo is reproducible by any judge, for free, with no coupon required.

## Tech stack

Qwen models via DashScope's OpenAI-compatible endpoint (no vendor SDK beyond `openai`), with per-role model routing overridable through environment variables so the DashScope catalog can drift without a code change. Deployment target is Alibaba Cloud Function Compute, free tier, custom runtime, Singapore region (`serve.py` behind an `fcapp.run` URL; a self-contained zip with vendored `openai` is already built). 238 tests pass with zero API key required, covering concurrency safety, the retry ladder, crash recovery, dispute/arbitration, resume, git safety rails, the executor backends, web search, and the web API.
